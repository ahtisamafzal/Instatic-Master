...
describe('capability route matrix', () => {
  ...
  it('gates AI provider/audit/chat routes and filters mutating tools', async () => {
    ...
    try {
      ...
      expect(readOnlyToolNames).toContain('site_list_documents')
      expect(readOnlyToolNames).toContain('site_read_document')
      expect(readOnlyToolNames).toContain('site_open_document')
      expect(readOnlyToolNames).not.toContain('site_insert_html')
