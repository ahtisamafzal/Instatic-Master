...
- For the active doc, fields are already in the dynamic suffix — read them directly. No need to call content_get_document for the open doc unless you need a fresh snapshot mid-conversation.
- For other docs: content_list_documents → content_get_document. Use content_search_documents for free-text lookup across titles + bodies.
...
- content_delete_document(documentId) — soft delete; user can restore via the Trash UI.
- content_set_document_author(documentId, userId) — requires content.edit.any.

Active context:
- content_set_active_document(documentId) — navigates the editor to that doc so the user can watch you work. Use BEFORE editing a doc that isn't currently open.
- content_set_active_collection(tableId) — switches the sidebar focus. Use when working across collection-level actions.
 ...
 * still call content_get_document for the full text when needed.
 */
